...
export default function OrderCard({
  ...
  return (
    <div key={order.id} className="relative flex items-center justify-between p-6 border border-gray-200 rounded-lg shadow-lg hover:shadow-xl transition">
        ...
        <div className="relative bg-white w-[190px] h-[180px] rounded-lg overflow-hidden">
          ...
          >
            <SwiperSlide className="h-full">
              <img src="/image.jfif" alt="" className="w-full h-full object-cover" />
            </SwiperSlide>
            <SwiperSlide className="h-full">
              <img src="/image.jfif" alt="" className="w-full h-[260px] object-cover" />
            </SwiperSlide>
            <SwiperSlide className="h-full">
              <img src="/image.jfif" alt="" className="w-full h-full object-cover" />
            </SwiperSlide>
        ...
        <div className="flex-grow space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-6 items-center">
              <span
                className={`px-3 py-2 rounded-full text-[14px] font-bold w-[150px] text-center ${order.status === "Confirmed"
            ...
            <div>
              <div className="flex gap-5 items-center">
                <button>
                  {order.isMessaged ? (
                    <img src="/chat.png" className="w-[20px] h-[20px] object-cover" />
                  ) : (
                    <img src="/chat (1).png" className="w-[20px] h-[20px] object-cover" />
                  )}
                </button>
      ...
      {showPopup === order.id && (
        <div className="absolute bg-white border rounded-lg text-[black]/70 shadow-lg p-3 mt-2 w-[200px] top-12 right-4">
          <div
            onClick={() => setShowDeletePopup(order.id)}
